...
# pi_plus meshes / default stance need slightly more clearance than k1 on stadium collision.
PI_PLUS_SPAWN_Z_EXTRA_M = 0.04
...
BLUE_TEAM_MESH_PREFIX = "blue_mesh_"


def _resolved_compiler_meshdir(robot_xml: Path) -> Path:
    root = ET.fromstring(robot_xml.read_text(encoding="utf-8"))
    comp = root.find("compiler")
    sub = "meshes/"
    if comp is not None and comp.get("meshdir"):
        sub = str(comp.get("meshdir")).strip()
    sub_path = Path(sub)
    if sub_path.is_absolute():
        return sub_path.resolve()
    return (robot_xml.parent / sub_path).resolve()


def _append_blue_team_mesh_clones(robot_asset: ET.Element, blue_mesh_dir: Path, name_prefix: str = BLUE_TEAM_MESH_PREFIX) -> None:
    missing: list[str] = []
    for child in list(robot_asset):
        ...
        if not name or not file_attr:
            continue
        fn = Path(file_attr).name
        src = blue_mesh_dir / fn
        if not src.is_file():
            missing.append(str(src))
            continue
        dup = ET.Element("mesh")
        dup.set("name", f"{name_prefix}{name}")
        dup.set("file", src.resolve().as_posix())
        scale = child.get("scale")
        if scale:
            dup.set("scale", scale)
        robot_asset.append(dup)
    if missing:
        raise RuntimeError(
            "Blue-team mesh directory is missing STL files required by the robot MJCF. "
            f"First missing: {missing[0]!r} (and {len(missing) - 1} more)."
        )


def _rewrite_body_tree_mesh_refs(body: ET.Element, prefix: str = BLUE_TEAM_MESH_PREFIX) -> None:
    for el in body.iter():
        if el.tag != "geom":
            continue
        m = el.get("mesh")
        if m:
            el.set("mesh", f"{prefix}{m}")


def _apply_soccer_team_geom_colors(body: ET.Element, team: str) -> None:
    """Recolor MJCF geoms per soccer side. Motrix web view uses geom rgba (no post-load tint API)."""
    if team not in ("red", "blue"):
        return
    for geom in body.iter("geom"):
        rgba_s = geom.get("rgba")
        if not rgba_s:
            continue
        parts = rgba_s.split()
        if len(parts) < 4:
        ...
        if "K1logo" in mesh:
            if team == "red":
                nr, ng, nb = 0.42, 0.1, 0.1
            else:
                nr, ng, nb = 0.08, 0.18, 0.48
        elif r < 0.08 and g < 0.08 and b < 0.08:
            continue
        else:
            if team == "red":
                nr, ng, nb = 0.82, 0.26, 0.21
            else:
                nr, ng, nb = 0.15, 0.36, 0.88
        geom.set("rgba", f"{nr:.5f} {ng:.5f} {nb:.5f} {a:.5f}")
...
def _build_multi_robot_soccer_scene_xml(
    ...
    spawn_z_extra_m: float = 0.0,
    robot_xml_blue: Path | None = None,
) -> tuple[Path, list[int]]:
    robot_root = ET.fromstring(robot_xml.read_text(encoding="utf-8"))
    world_root = ET.parse(soccer_world_xml).getroot()

    k1_bundle = robot_xml.name.lower().startswith("k1") or robot_xml.resolve().parent.name.lower() == "k1"
    meshes_blue_dir = robot_xml.parent / "meshes_blue"
    stl_count = (
        sum(
            1
            for p in meshes_blue_dir.iterdir()
            if p.is_file() and p.suffix.lower() == ".stl"
        )
        if meshes_blue_dir.is_dir()
        else 0
    )
    has_blue_stl = stl_count > 0
    use_meshes_blue_pack = k1_bundle and has_blue_stl
    meshdir = (robot_xml.parent / "meshes_blue") if use_meshes_blue_pack else (robot_xml.parent / "meshes")
    print(
        f"[MotrixSim scene] mjcf={robot_xml.name} parent={robot_xml.parent.name} "
        f"meshdir->{meshdir.name}/ (k1_pack={k1_bundle} meshes_blue_stl={stl_count} use_meshes_blue={use_meshes_blue_pack})"
    )
    ...
    blue_body_template: ET.Element | None = None
    if robot_xml_blue is not None and max_blue_robots > 0:
        blue_mesh_dir = _resolved_compiler_meshdir(robot_xml_blue)
        if blue_mesh_dir.is_dir() and blue_mesh_dir.resolve() != meshdir.resolve():
            _append_blue_team_mesh_clones(robot_asset, blue_mesh_dir)
            blue_body_template = deepcopy(template_body)
            _rewrite_body_tree_mesh_refs(blue_body_template)
    ...
    def add_team(team: str, count: int):
        ...
        for i in range(count):
            ...
            if team == "blue" and blue_body_template is not None:
                body_copy = deepcopy(blue_body_template)
            else:
                body_copy = deepcopy(template_body)
            _prefix_body_tree_names(body_copy, robot_name)
            _apply_soccer_team_geom_colors(body_copy, team)
            ...
            spawn_z = float(template_spawn_z) + float(ROBOT_SPAWN_Z_LIFT_M) + float(spawn_z_extra_m)
...
class MultiRobotMotrixSim:
    def __init__(self, args: RuntimeArgs):
        ...
        pi_plus_spawn_z_extra = (
            PI_PLUS_SPAWN_Z_EXTRA_M if self.robot_cfg.robot_type == PI_PLUS_ROBOT_TYPE else 0.0
        )
        k1_blue_xml = (
            args.robot_xml_blue
            if self.robot_cfg.robot_type == K1_ROBOT_TYPE and args.robot_xml_blue is not None
            else None
        )
        scene_xml, _ = _build_multi_robot_soccer_scene_xml(
            ...
            spawn_z_extra_m=pi_plus_spawn_z_extra,
            robot_xml_blue=k1_blue_xml,
        )
